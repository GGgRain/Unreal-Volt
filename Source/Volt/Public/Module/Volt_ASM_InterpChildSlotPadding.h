...
public:
	...
	VOLT_MODULE_BEGIN_ARGS(UVolt_ASM_InterpChildSlotPadding) :
		_InterpolationMode(EVoltInterpMode::RateBased),
		
		_StartPadding(FMargin()),
		_bUseStartPadding(false),
		_TargetPadding(FMargin()),
	...
	{}
		...
		VOLT_MODULE_ARGUMENT( FMargin, StartPadding )
		VOLT_MODULE_ARGUMENT( bool, bUseStartPadding )
		VOLT_MODULE_ARGUMENT( FMargin, TargetPadding )
...
public:
	...
	/**
	 * Start Padding of the animation. The color of the slate will be overrided with it if bUseStartColor was true.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="Common")
	FMargin StartPadding;

	/**
	 * Whether to override the Padding with StartPadding when the module has been started.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="Common")
	bool bUseStartPadding = false; //If not specified, do nothing.

	/**
	 * Target Padding to interpolate to.
	 */
